...
import QRCodeDetailLayout from "../components/QRCodeDetailLayout/QRCodeDetailLayout.tsx";
...
function FaWuPage() {
    ...
    useEffect(() => {
        try {
            const getList = async () => {
                const apiUrl = "https://fsy.zhifo.net.cn/fosiyun/api/v1/operatingFloor/item/list?tenantId=360&type=2"
                ...
                if (data.status !== 0) {
                    throw new Error(`获取法物登记列表数据异常，错误码：${data.status}`);
                }
                ...
                if (Array.isArray(fetchData)) {
                    ...
                    throw new Error(`获取法物登记列表数据异常，原因：fetchData 不是数组`);
                }
            }

            getList()
        } catch (e) {
            console.error("获取法物登记列表失败，原因：", e)
        }
    }, []);
    ...
    return (
        <>
            { openDetail ? (
                <QRCodeDetailLayout detailInfo={detailInfo} />
            ) : (
                <CardLayout list={list} onOpenItemDetail={openDetailHandler} />
            ) }
        </>
    );
}

export default FaWuPage;
